...
namespace traitement{
  ...
  Annotation::Annotation(std::string file){
    ...
    cv::Scalar color1 = {root["color_team_1"]["r"].asUInt(),
                         root["color_team_1"]["g"].asUInt(),
                         root["color_team_1"]["b"].asUInt()};

    cv::Scalar color2 = {root["color_team_2"]["r"].asUInt(),
                         root["color_team_2"]["g"].asUInt(),
                         root["color_team_2"]["b"].asUInt()};
  ...
  cv::Mat  Annotation::annotePosition(CameraMetaInformation camera_information,
    ...
    if (annotation_choice["optimized"]){
      ...
      if (opacity > 0){
        ...
        cv::circle(overlay, pos_in_img, sizecircle, colorAnnotation, cv::FILLED);
        cv::addWeighted(overlay, opacity, display, 1-opacity, 0, display);
      }
    }
    else{
      cv::circle(display,pos_in_img, sizecircle, colorAnnotation, cv::FILLED);
    }
  ...
  cv::Mat  Annotation::annoteTarget(CameraMetaInformation camera_information,
            RobotMsg robot ,Position pos_target,cv::Mat display,  uint64_t now){
  ...
  cv::Mat  Annotation::annoteDirection( CameraMetaInformation camera_information,
    ...
    if (annotation_choice["optimized"]){
      ...
      if (opacity > 0){
        cv::Mat overlay;
        display.copyTo(overlay);
        cv::arrowedLine(overlay, pos_in_img, fleche, colorAnnotation, 2, 0, 0.1);
        cv::addWeighted(overlay, opacity, display, 1-opacity, 0, display);
      }
    }
    else {
