...
int main() {
  ...
  fprintf(csv,
    "Time to update messages;"
    "Time getCalibratedImage;"
    "Time cloneCalibratedImage;"
    "Time Annotations Field&Score;"
    "Time addAnnotations;"
    "Total elapsed_time;"
  ...
  //initialized at the begining of the programm, used to regulate the time
  auto begin = std::chrono::system_clock::now();
  //initialized at the begining of the loop and used at the end
  auto loop = std::chrono::system_clock::now();
  //used for calculs
  auto start = std::chrono::system_clock::now();
  //always have the latest time
        ...
        if(!speed_optimized){
          ...
          if (entry.first != images_by_source.begin()->first
            ...
            if (elapsed_useconds.count() >= (now - begin_time) - SECONDS_TO_MS){
              ...
              cv::waitKey(((now - begin_time) - elapsed_useconds.count())
                            / SECONDS_TO_MS);
              fprintf(csv, "slow \n");
            }
