...
MainWindow::MainWindow(){
  ...
  gamePicture = new Mat(CV_IMG_WIDTH,CV_IMG_HEIGHT,CV_8UC4,Scalar(0,0,255));
  cvtColor(*gamePicture, *gamePicture, CV_BGR2RGB);
  ...
  labelVideo->setPixmap(QPixmap::fromImage(QImage(gamePicture->data,
                        gamePicture->cols, gamePicture->rows,
                        gamePicture->step, QImage::Format_RGB888)));

  sliderValue = new QLabel(this);
  sliderValue->setText("0");
  slider = new QSlider(Qt::Horizontal, this);

  buttonAnnotationChoice = new QPushButton("Annotation Choice");
  buttonPause = new QPushButton("PAUSE");
  buttonFastForward = new QPushButton(">>");

  connect(buttonAnnotationChoice, SIGNAL(released()),
          this, SLOT(annotationChoice()));
  connect(buttonPause, SIGNAL(released()),
          this, SLOT(togglePause()));
  connect(buttonFastForward, SIGNAL(released()),
          this, SLOT(toggleFastForward()));
  ...
  layout->addWidget(buttonAnnotationChoice,8,4,1,1);
...
void MainWindow::changeImage(){
  ...
  if(!boolPause){
    ...
    if (manager.isLive())
      ...
      uint64_t totalDuration = endTime - initialTime;
      float targetTimeStamp = (totalDuration * slider->value())/SLIDER_INTERVAL;

      if(slider->value() > oldSliderValue){
        while(now - initialTime < targetTimeStamp){
          now += FRAME_DURATION;
        }
      }
      ...
      now += FRAME_DURATION;
      slider->setValue((now - initialTime)*SLIDER_INTERVAL/totalDuration);
    ...
    for(const auto & entry : images_by_source){
      ...
      Mat display_img = entry.second.getImg().clone();
      if (entry.second.isFullySpecified()){
        ...
        for(const auto & robot_entry : status.robot_messages){
          ...
          teams[team_id].updateRobot(robot_entry.first.robot_id(),
                                     robot_entry.second);

          display_img = annotation->addAnnotation(camera_information,
                        teams[team_id].getRobot(robot_entry.first.robot_id()) ,
                        display_img,
                        now
                        );
        }
      ...
      cvtColor(display_img, display_img, CV_BGR2RGB);
      this->labelVideo->setPixmap(QPixmap::fromImage(QImage(display_img.data,
                                                       display_img.cols,
                                                       display_img.rows,
                                                       display_img.step,
  ...
  for (auto it : teamPanels){
    (it.second)->updateAnnotation(
                   boolPosition, boolDirection, boolTrace, boolBall, boolTarget,
...
void MainWindow::annotationChoice(){
  ChoiceDialog * msgBox = new ChoiceDialog();

  msgBox->setInitGeneral(boolPosition, boolDirection, boolTrace,
                         boolBall, boolTarget);
  msgBox->setTeamMap(teams);
  msgBox->setCurrentTrace(annotation->getTeamTrace(),
                          annotation->getRobotTrace());
  msgBox->setCurrentBall(annotation->getTeamBall(),
                         annotation->getRobotBall());
  msgBox->setCurrentTarget(annotation->getTeamTarget(),
                           annotation->getRobotTarget());
